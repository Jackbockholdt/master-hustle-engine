...
async function runAutonomousDailyIntake({
  ...
  let currentQueueDepth = getLeadQueueDepth();
  console.log(`[Intake Telemetry] Current lead queue depth: ${currentQueueDepth} uncontacted leads.`);

  const targetQueueDepth = minQueueDepth ? parseInt(minQueueDepth, 10) : null;
  const targetNewLeads = parseInt(limit, 10) || 15;

  // Determine query list to cycle through
  const dayIndex = new Date().getDate() % ICP_QUERIES.length;
  const queriesToRun = query 
    ? [query] 
    : [...ICP_QUERIES.slice(dayIndex), ...ICP_QUERIES.slice(0, dayIndex)];

  const outscraperApiKey = (process.env.OUTSCRAPER_API_KEY || '').trim();
  const outreachHistory = loadOutreachHistory();
  const blocklist = loadBlocklist();

  const totalStagedLeads = [];
  const totalSkippedLeads = [];
  const totalDisqualifiedMxLeads = [];
  let totalEvaluatedCount = 0;
  const inBatchDomains = new Set();
  const inBatchEmails = new Set();

  let queryIndex = 0;
  let lastUsedSource = 'CURATED_DIRECTORY_FALLBACK';

  while (queryIndex < queriesToRun.length) {
    const selectedQuery = queriesToRun[queryIndex];
    queryIndex++;

    const neededMore = targetQueueDepth 
      ? Math.max(0, targetQueueDepth - getLeadQueueDepth())
      : Math.max(0, targetNewLeads - totalStagedLeads.length);

    if (neededMore <= 0 && (totalStagedLeads.length > 0 || currentQueueDepth >= (targetQueueDepth || 1))) {
      console.log(`[Intake Target Satisfied] Target satisfied. Current queue depth: ${getLeadQueueDepth()}`);
      break;
    }

    const fetchLimit = Math.min(30, Math.max(10, neededMore + 10));
    console.log(`\n[Intake Sourcing] Query (${queryIndex}/${queriesToRun.length}): "${selectedQuery}" (Fetch limit: ${fetchLimit})`);
    ...
    if (outscraperApiKey && !mock) {
      try {
        ...
        if (Array.isArray(places) && places.length > 0) {
          ...
          rawPlaces = places;
          lastUsedSource = 'OUTSCRAPER_LIVE_API';
        } else {
          console.log(`[Outscraper API] Query returned 0 places.`);
        }
      } catch (err) {
    ...
    if (rawPlaces.length === 0 && (mock || !outscraperApiKey)) {
    ...
    console.log(`[Intake Evaluation] ${candidateLeads.length} normalized candidates ready for dedupe and MX check.`);
    totalEvaluatedCount += candidateLeads.length;

    for (const cand of candidateLeads) {
      if (targetQueueDepth && getLeadQueueDepth() >= targetQueueDepth) {
        break;
      }
      if (!targetQueueDepth && totalStagedLeads.length >= targetNewLeads) {
        break;
      }
      ...
      if (inBatchEmails.has(email) || inBatchDomains.has(domain)) {
        totalSkippedLeads.push({ email, company, reason: 'IN_BATCH_DUPLICATE' });
        continue;
      }

      const dbCheck = isLeadInPipeline({ email, domain, company });
      if (dbCheck.exists) {
        totalSkippedLeads.push({ email, company, reason: `DUPLICATE_PIPELINE_DB_${dbCheck.match.toUpperCase()}` });
        continue;
      }

      if (outreachHistory.emails.has(email) || outreachHistory.domains.has(domain)) {
        totalSkippedLeads.push({ email, company, reason: 'DUPLICATE_OUTREACH_LOG' });
        continue;
      }

      if (blocklist.emails.has(email) || blocklist.domains.has(domain)) {
        totalSkippedLeads.push({ email, company, reason: 'SUPPRESSED_BLOCKLIST' });
        continue;
      }

      // Live RFC Syntax and DNS/MX Verification
      ...
      if (!preFlight.valid) {
        if (preFlight.reason === 'DISQUALIFIED_INVALID_MX' && !dryRun) {
          ...
          totalDisqualifiedMxLeads.push({ email, company, reason: preFlight.reason });
        } else {
          totalSkippedLeads.push({ email, company, reason: preFlight.reason });
        }
        continue;
      }
      ...
      if (!dryRun) {
        ...
        console.log(`[Intake Staged] ${email} (${company}) -> pipeline.db [discovered]`);
      }

      totalStagedLeads.push(stagePayload);
    }

    if (mock) break; // In mock mode only run first query
  }
  ...
  console.log(`  - Sourcing Source: ${lastUsedSource}`);
  console.log(`  - Queries Run:     ${queryIndex}`);
  console.log(`  - Total Staged:    ${totalStagedLeads.length}`);
  console.log(`  - Dedupe Skips:    ${totalSkippedLeads.length}`);
  console.log(`  - MX Disqualified: ${totalDisqualifiedMxLeads.length}`);
  console.log(`  - Final Queue Depth: ${finalQueueDepth} uncontacted qualified leads`);
  console.log(`  - Execution Time:  ${executionTimeMs}ms`);
  console.log(`===================================================================`);

  return {
    success: true,
    sourcingSource: lastUsedSource,
    queriesRun: queryIndex,
    evaluatedCount: totalEvaluatedCount,
    dedupeSkipsCount: totalSkippedLeads.length,
    stagedCount: totalStagedLeads.length,
    stagedLeads: totalStagedLeads,
    skippedCount: totalSkippedLeads.length,
    disqualifiedMxCount: totalDisqualifiedMxLeads.length,
