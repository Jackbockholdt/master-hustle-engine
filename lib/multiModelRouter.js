 ...
 * Format: "gemini:KEY1,claude:KEY2,grok:KEY3,openrouter:KEY4"
 */
  ...
  if (raw) {
    ...
    for (const entry of entries) {
      ...
      if (idx > 0) {
        ...
        pool.push({ provider, apiKey, model: PROVIDER_MODELS[provider] });
      }
    }
  }

  // Fall back to individual env variables if pool is empty or missing providers
  const hasProvider = (p) => pool.some(e => e.provider === p);
  
  if (!hasProvider('gemini') && process.env.GEMINI_API_KEY) {
    pool.push({ provider: 'gemini', apiKey: process.env.GEMINI_API_KEY, model: PROVIDER_MODELS.gemini });
  }
  if (!hasProvider('claude') && (process.env.ANTHROPIC_API_KEY || process.env.CLAUDE_API_KEY)) {
    pool.push({ provider: 'claude', apiKey: process.env.ANTHROPIC_API_KEY || process.env.CLAUDE_API_KEY, model: PROVIDER_MODELS.claude });
  }
  if (!hasProvider('grok') && (process.env.GROK_API_KEY || process.env.XAI_API_KEY)) {
    pool.push({ provider: 'grok', apiKey: process.env.GROK_API_KEY || process.env.XAI_API_KEY, model: PROVIDER_MODELS.grok });
  }
  if (!hasProvider('openrouter') && process.env.OPENROUTER_API_KEY) {
 ...
 */
async function dispatchClaude({ apiKey, model }, prompt, systemPrompt) {
...
function getRouterStatus() {
  ...
  return {
    status: 'healthy',
    configuredProviders: pool.map(p => p.provider),
