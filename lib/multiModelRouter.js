 ...
 * Format: "gemini:KEY1,openai:KEY2,claude:KEY3,grok:KEY4,openrouter:KEY5"
 */
function parseApiPool() {
  ...
  if (!seen.has('openai') && process.env.OPENAI_API_KEY) {
    seen.add('openai');
    pool.push({ provider: 'openai', apiKey: process.env.OPENAI_API_KEY, model: PROVIDER_MODELS.openai });
  }

  // Secondary/Fallback: Claude / Anthropic
 ...
 */
async function dispatchOpenAI({ apiKey, model }, prompt, systemPrompt) {
  ...
  return { text, provider: 'openai', model: model || 'gpt-4o-mini' };
}
...
function getRouterStatus() {
  ...
  const primaryProvider = configured[0] || 'gemini';
  const secondaryProvider = configured[1] || (process.env.OPENAI_API_KEY ? 'openai' : (process.env.ANTHROPIC_API_KEY || process.env.CLAUDE_API_KEY ? 'claude' : 'not_configured'));
