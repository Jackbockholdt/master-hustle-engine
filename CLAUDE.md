...
SQLite lives on the Render persistent disk: **`/var/data/my_database.db`** (disk `data_storage`, mounted at `/var/data`). The suppression list, lead queue, follow-ups, and send counts survive deploys. If the disk isn't mounted (local dev), the engine falls back to a repo-local `transactions.sqlite`. Overrides: `DB_PATH` (full file path) or `DATA_DIR` (mount dir) env vars. Before this, every deploy silently wiped the database — never move the DB back inside the repo directory.
